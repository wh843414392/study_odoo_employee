...
var crypt = require('web.Crypt');

var reg = /session_id=([^;]+)(?:;|$)/;

var cryptUrls = ['/web/action/load', '/web/dataset/search_read', '/web/dataset/load',
                 '/web/dataset/call', '/web/dataset/call_kw', '/web/dataset/call_button',
                 '/web/dataset/exec_workflow', '/web/dataset/resequence', '/web/view/add_custom',
                 '/web/treeview/action'];
...
function genericJsonRpc (fct_name, params, fct) {
    ...
    var result = xhr.pipe(function(result) {
        ...
        if(result.hasOwnProperty('data') && result.hasOwnProperty('summary')) {

            var MD5Data = crypt.MD5(result.data),
                newMD5Data = crypt.MD5(MD5Data + session_id + '/H0w<c|:dYV(.q)b');

            if(result.summary === newMD5Data) {

                result = JSON.parse(crypt.Decrypt(result.data, "f4BFP3IbxegslKVF0jJ0bXMRKrk9iocXCrY+6sPvPaw="));
...
function jsonRpc(url, fct_name, params, settings) {
    return genericJsonRpc(fct_name, params, function(data) {
        ...
        if(!(window.location.host.indexOf('localhost') != -1
             ...
             || window.location.host.indexOf('0.0.0.0') != -1) && (cryptUrls.indexOf(url) !== -1 || url.match('/web/dataset/call_kw'))) {
        // if(false) {
            var newData = crypt.Encrypt(JSON.stringify(data, time.date_to_utc), "f4BFP3IbxegslKVF0jJ0bXMRKrk9iocXCrY+6sPvPaw=");
            var MD5Data = crypt.MD5(newData);
            var NewMD5Data = crypt.MD5(MD5Data + session_id + '/H0w<c|:dYV(.q)b');

            data = JSON.stringify({data: newData, summary: NewMD5Data});
            headers.identification = crypt.base64.encode(JSON.stringify(identification));
