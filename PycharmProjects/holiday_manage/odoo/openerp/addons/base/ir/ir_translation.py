...
class ir_translation_import_cursor(object):
    ...
    def finish(self):
        ...
        if self._overwrite:
            #20161020 周兵 只对is_customer＝false的数据进行更新，以保存证不影响已上线客户确认过的翻译
...
class ir_translation(osv.osv):
    ...
    #20161020 周兵 需要进行_log_access，以记录create_uid,create_data,write_uid,write_date
    _log_access = True
    ...
    _columns = {
        ...
        #周兵 20161020 增加下面的字段以例对定制化的翻译不进入变更等控制，公共翻译功能还未实现。
        'is_customer': fields.boolean(string='Is Customer',select=True),
        'is_public': fields.boolean(string='Is Public',select=True),
    }
    ...
    def _sync_terms_translations(self, field, records):
        ...
        for record in records:
            ...
            record_trans = trans.search([
                ...
                ('name', '=', "%s,%s" % (field.model_name, field.name)),
                ('res_id', '=', record.id),
                #周兵 20161020 同步术语时也只对未定制化的翻译进行操作，不影响客户已确认的数据。
                ('is_customer', '=', False),
            ])
    ...
    def insert_missing(self, field, records):
        ...
        if callable(field.translate):
            ...
            #周兵 20161020 新增的数据给缺省值
            query = """ INSERT INTO ir_translation (lang, type, name, res_id, src, value, module, is_customer, is_public, create_uid, create_date, write_uid, write_date)
                        SELECT l.code, 'model', %(name)s, %(res_id)s, %(src)s, %(src)s, %(module)s, False, False, 1, now(), 1, now()
                        FROM res_lang l
                        WHERE NOT EXISTS (
    ...
    def is_include_module(self, modpath):
        """
        周兵 20161020
        增加此方法来允许安装升级可选模块加载翻译。
        :param modpath:
        :return:
        """
        ret_bool = False
        lang_module_dirs = openerp.tools.config.get('lang_module_dirs','all')
        if lang_module_dirs:
            if lang_module_dirs=='all':
                return True
            for mod_dir in lang_module_dirs.split(','):
                if mod_dir in modpath:
                    return True
        return ret_bool

    def load_module_terms(self, cr, modules, langs, context=None):
        context_template = dict(context or {}) # local copy
        #20161016 周兵 增加配置，ignore_lang，如果存在则不进行翻译导入操作。缺省是要加载翻译
        ignore_lang = openerp.tools.config.get('ignore_lang',False)
        _logger.info('ignore_lang:%s' %(ignore_lang,))
        if ignore_lang:
            _logger.info('ignore load lang file, modules:%s' %(modules,))
            return True
        for module_name in modules:
            #20161016 周兵 检查是否在配置项里声明过的模块目录，如不在声明里则不加载，缺省是要加载模块的翻译
            modpath = openerp.modules.get_module_path(module_name)
            if not modpath:
                continue
            #20161016 周兵 仅对设定的目录进行翻译文件装载操作
            if not self.is_include_module(modpath):
                _logger.info('ignore load po file, module_name:%s , path:%s' %(modules,modpath))
                continue
